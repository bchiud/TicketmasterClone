...
- **Against**:
  - Not a real association — no foreign-key integrity. The array can hold a
    ticket id that doesn't exist, or the *same* ticket id in two bookings, with
    nothing at the DB layer to stop it.
  - No navigation from a ticket back to its booking.
  - Awkward in JPA: needs a custom array type/converter, no lazy loading, no
    cascade.
  - "Which booking holds ticket X?" becomes an array-containment scan instead of
    an indexed FK lookup.
  - Throws away exactly the relational guarantees ADR-0002 chose Postgres for.
...
- **For**:
  ...
  - No field on `Ticket`.
- **Against**:
  - A join table is the idiomatic shape for *many-to-many*, not one-to-many.
  - Doesn't enforce one-booking-per-ticket without an extra unique constraint on
    the ticket column.
  - Adds a table plus an extra join for no benefit.
  - Was effectively the pre-refactor state — and worse, it coexisted with an
    unused `Ticket.booking @ManyToOne`, so the relationship was mapped *twice*:
    the join table (actually populated) plus a dead `tickets.booking_id` FK
    column that no code ever set.
...
- **For**:
  - The idiomatic one-to-many mapping.
  - The FK enforces referential integrity and, being single-valued on the
    ticket, structurally guarantees a ticket belongs to at most one booking.
  - Navigable both ways; supports lazy loading and the `getTickets()` the sweeps
    rely on.
  - No join table.
  - Reuses the `booking_id` column that already existed (previously dead),
    turning redundant schema into the single source of truth.
- **Against**:
  - The owning side is `Ticket.booking`, so application code must set it
    (`ticket.setBooking(booking)`) — setting only `booking.setTickets(...)` (the
    inverse side) writes nothing.
  - Bidirectional mappings carry the usual "keep both sides consistent" burden.
  - The bidirectional graph has no Jackson stop-condition, so serializing a
    `Booking` entity recurses `Booking → tickets → booking → …`; the API layer
    must therefore not serialize the entities directly (resolved by
    [ADR-0005](0005-response-dtos-osiv.md)).
...
- A released ticket (`expire()`/`cancel()`) has its `booking_id` nulled
  (`ticket.setBooking(null)`) so it no longer references the now-terminal
  booking.
- design.md §10's `bookings.ticket_ids BIGINT[]` has been superseded by the
  `tickets.booking_id` FK, and §10 updated to match.
